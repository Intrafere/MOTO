...
""")
    ...
    # CRITICAL FIX: Conclusion/Introduction/Abstract phases MUST see the current paper
    # to write proper summaries. Only skip paper for first body section (nothing written yet).
    requires_paper_context = section_phase in ("conclusion", "introduction", "abstract")
    
    if is_first_portion and not requires_paper_context:
        # Only for first body section - no paper exists yet to show
        if section_phase == "body":
            parts.append("TASK: Write the FIRST body section of the paper following the outline.")
        else:
            parts.append("TASK: Write the FIRST portion of the document that aligns with the outline.")
    else:
        # ALWAYS include paper for:
        # 1. Body continuations (is_first_portion=False)
        # 2. Conclusion phase (needs to see body sections to summarize)
        # 3. Introduction phase (needs to see body+conclusion to describe)
        # 4. Abstract phase (needs to see everything to summarize)
