...
def sanitize_json_response(raw_content: str) -> str:
    ...
    4. Try parsing as-is (fast path for valid JSON)
    5. If that fails, escape unescaped backslashes
    6. Handle common invalid escape sequences from LaTeX notation
        ...
        try:
            ...
            if start_char == '{':
                ...
                if end_pos > 0 and end_pos < len(content):
                    ...
                    # JSON object was never closed - likely LLM output was truncated
                    logger.warning(f"JSON object appears truncated: {brace_count} unclosed braces, in_string={in_string}")
                    # Try to detect where the issue started
                    last_complete = content.rfind('",')  # Last completed property
                    if last_complete > json_start:
                        logger.warning(f"Last completed property appears around char {last_complete}: ...{repr(content[max(0,last_complete-50):last_complete+50])}...")
                ...
                if end_pos > 0 and end_pos < len(content):
                    ...
                    # JSON array was never closed - likely LLM output was truncated
                    logger.warning(f"JSON array appears truncated: {bracket_count} unclosed brackets, in_string={in_string}")
    ...
    # STEP 4.8: Repair trailing newline spam and general truncation
    # Some models output coherent JSON content but then fill remaining tokens with \n\n\n...
    # or simply truncate mid-output without closing the JSON structure.
    # Detect and repair both patterns.
    
    # HELPER FUNCTION: Analyze JSON structure and compute repair needs
    def analyze_and_repair_json(text):
        """Analyze JSON structure and return (repaired_text, was_repaired)."""
        if not text:
            return text, False
        ...
        if open_braces == 0 and open_brackets == 0 and not in_string:
            return text, False  # Already complete
        
        # Need repair - build suffix
        repair_suffix = ''
        
        if in_string:
            # We're inside an unclosed string - close it
            repair_suffix += '"'
            logger.info(f"Repairing truncated JSON: closing unclosed string")
        
        # Close any remaining open braces/brackets
        repair_suffix += ']' * max(0, open_brackets)
        repair_suffix += '}' * max(0, open_braces)
        
        if repair_suffix:
            logger.info(f"Repaired truncated JSON: added '{repair_suffix}' (had {open_braces} unclosed braces, {open_brackets} unclosed brackets, in_string={in_string})")
            return text + repair_suffix, True
        
        return text, False
    
    # Pattern 1: Check for escaped newline spam (\n\n\n... in string → \\n\\n\\n in JSON)
    trailing_newline_match = re.search(r'((?:\\n){10,})$', content)
    
    # Pattern 2: Actual newline character spam
    if not trailing_newline_match:
        trailing_newline_match = re.search(r'(\n{10,})$', content)
    
    if trailing_newline_match:
        newline_start = trailing_newline_match.start()
        matched_text = trailing_newline_match.group(1)
        # Count actual newlines
        if matched_text.startswith('\\n'):
            newline_count = len(matched_text) // 2
        else:
            newline_count = len(matched_text)
        logger.warning(f"Detected trailing spam pattern: {newline_count} chars at position {newline_start}")
        
        # Strip the spam and repair
        content = content[:newline_start]
        content, repaired = analyze_and_repair_json(content)
    else:
        # No trailing spam detected, but still try to repair if JSON is incomplete
        # This handles simple truncation cases
        content, repaired = analyze_and_repair_json(content)
        
        if repaired:
            logger.info("Repaired truncated JSON without trailing spam (simple truncation)")
